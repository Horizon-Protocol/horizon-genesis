...
import {
  historicalClaimHZNAndZUSDAtom,
  historicalDebtAtom,
  historicalOperationAtom,
  HistoryType,
} from "@atoms/record";
...
export default function useQueryDebt() {
  ...
  useQuery([GRAPH_DEBT, "historyDebt"], fetcher, {
    // initialData: [],
    onSuccess([issues, burns, claims, debtSnapshot]) {
      let issuesAndBurns = issues.issueds!.map((b: any) => ({
        isBurn: false,
        ...b,
      }));
      issuesAndBurns = sortBy(
        issuesAndBurns.concat(
          burns.burneds!.map((b: any) => ({ isBurn: true, ...b }))
        ),
        (d) => {
          return d.timestamp;
        }
      ...
      /* --------------------------- */
      const typeMintHistory = issues.issueds.map((b: any) => ({
        ...b,
        type: HistoryType.Mint,
      }));
      const typeBurnHistory = burns.burneds.map((b: any) => ({
        ...b,
        type: HistoryType.Burn,
      }));
      const typeClaimHistory = claims.feesClaimeds.map((b: any) => ({
        ...b,
        type: HistoryType.Claim,
      }));
      const allTypeHistory = sortBy(
        concat(typeMintHistory, typeBurnHistory, typeClaimHistory),
        "timestamp"
      );
      ...
      const historicalDebtAndIssuance: HistoricalDebtAndIssuanceData[] = [];
      debtHistory
        .slice()
        .reverse()

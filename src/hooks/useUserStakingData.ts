...
import { feePoolAtom } from "@atoms/feePool";
...
export default function useUserStakingData() {
  ...
  const { feesToDistribute, rewardsToDistribute, rewardsToDistributeBN } =
    useAtomValue(feePoolAtom);
  ...
  const weeklyRewards = zUSDRate
    ...
    .plus(hznRate.multipliedBy(rewardsToDistribute));
    ...
    return {
      currentFeePeriodStarts: new Date(
        currentFeePeriod.data?.startTime
          ? currentFeePeriod.data.startTime * 1000
          : 0
      ),
